// Bu dosya, Django Rest Framework'ün API dokümantasyonunda kullanılan ana JavaScript dosyasıdır.
// API Explorer arayüzünde istek gönderme, yanıtları gösterme, kimlik doğrulama ve dil seçimi gibi işlevleri yönetir.

var responseDisplay = 'data'
var coreapi = window.coreapi
var schema = window.schema
// Düzleştirilmiş anahtar dizisi oluşturur (ör: ['foo > bar'] => ['foo', 'bar'])
...
// HTTP header isimlerini düzgün şekilde büyük harfe çevirir
...
// Form verilerini anahtar-değer çiftlerine dönüştürür
...
$(function () {
  // Kimlik doğrulama ve modal referansları
  ...
  $('#language-control li').click(function (event) {
   ...
   // API Explorer form gönderimi ve yanıt işlemleri
  $('form.api-interaction').submit(function (event) {
    ...
    // Formdan gelen verileri uygun tipe dönüştür
...
// Seçili olmayan checkbox'ları false olarak ekle
    $form.find(':checkbox').each(function (index) {
     ...
     // İstek bilgilerini ekrana yazdıran fonksiyon
    ...
    // Yanıt bilgilerini ekrana yazdıran fonksiyon
    ...
    // API isteği için coreapi client'ı oluştur ve isteği gönder
    var options = {
      requestCallback: requestCallback,
      responseCallback: responseCallback
    }

    // Kimlik doğrulama seçeneklerini ayarla
    if (window.auth && window.auth.type === 'token') {
      // Token ile header authentication
      ...
      options.auth = new coreapi.auth.BasicAuthentication({
        ...
        // Session authentication (CSRF korumalı)
